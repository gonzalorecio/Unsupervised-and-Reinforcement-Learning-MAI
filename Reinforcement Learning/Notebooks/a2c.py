...
class A2C(object):
    def __init__(self, env, args):
        ...
        self.optimizer_critic = optim.Adam(
            self.critic.parameters(), lr=args.lr_critic)
    ...
    def optimize(self, rewards, log_probs, values):
        ...
        for t in reversed(range(T)):
            ...
            R[t] = (self.gamma**N * V_end) + sum([self.gamma**k *
                                                  rewards[t+k]*1e-2 for k in range(min(N, T-t))])
        ...
        for e in range(num_episodes):
            ...
            if (e+1) % (self.save_freq) == 0:
                torch.save(self.actor.state_dict(),  os.path.join(
                    self.save_path, "train_actor_ep_{}.pkl".format(e+1)))
                torch.save(self.critic.state_dict(), os.path.join(
                    self.save_path, "train_critic_ep_{}.pkl".format(e+1)))
    ...
    def test(self, num_episodes, e_train):
        ...
        if np.mean(testing_rewards) >= 200:
            print("-"*10 + " Solved! " + "-"*10)
            print("Mean reward achieved : {} in {} steps".format(
    ...
    def plot_rewards(self, save=False):
        train_rewards = [self.train_rewards[i:i+self.test_freq]
                         for i in range(0, len(self.train_rewards), self.test_freq)]
        test_rewards = [self.test_rewards[i:i+self.test_episodes]
                        for i in range(0, len(self.test_rewards), self.test_episodes)]
        train_losses_actor = [self.losses_actor[i:i+self.test_freq]
                              for i in range(0, len(self.losses_actor), self.test_freq)]
        train_losses_critic = [self.losses_critic[i:i+self.test_freq]
                               for i in range(0, len(self.losses_critic), self.test_freq)]
        train_losses = [self.losses_critic[i:i+self.test_freq]+self.losses_actor[i:i +
                                                                                 self.test_freq] for i in range(0, len(self.losses_critic), self.test_freq)]
        ...
        train_nepisodes = [self.test_freq * (i+1)
                           for i in range(len(train_rewards_mean))]

        # steps
        train_steps = [self.train_steps[i:i+self.test_freq]
                       for i in range(0, len(self.train_steps), self.test_freq)]
        test_steps = [self.test_steps[i:i+self.test_episodes]
                      for i in range(0, len(self.test_steps), self.test_episodes)]
        ...
        plt.figure(1)
        plt.clf()
        plt.title("Training : Avg. Reward over {} episodes".format(
            self.test_episodes))
        plt.xlabel("Number of training episodes")
        plt.ylabel("Avg Reward")
        plt.errorbar(train_nepisodes, train_rewards_mean,
                     yerr=train_rewards_std, color="indigo", uplims=True, lolims=True)
        if save:
            plt.savefig(self.expt_name +
                        "train_rewards_{}.png".format(len(self.train_rewards)))
        else:
            plt.show()
        ...
        plt.figure(2)
        plt.clf()
        plt.title("Testing : Avg. Reward over {} episodes".format(
            self.test_episodes))
        plt.xlabel("Number of training episodes")
        plt.ylabel("Avg Reward")
        try:
            plt.errorbar(train_nepisodes, test_rewards_mean,
                         yerr=test_rewards_std, color="indigo", uplims=True, lolims=True)
        except:
            ipdb.set_trace()
        if save:
            plt.savefig(self.expt_name +
                        "test_rewards_{}.png".format(len(self.test_rewards)))
        else:
            plt.show()
        ...
        plt.errorbar(train_nepisodes, train_steps_mean,
                     yerr=train_steps_std, color="navy", uplims=True, lolims=True)
        if save:
            plt.savefig(self.expt_name +
                        "train_steps_{}.png".format(len(self.train_steps)))
        else:
            plt.show()
            # pause so that the plots are updated
            plt.pause(0.001)
        ...
        plt.xlabel("Number of training episodes")
        plt.ylabel("Avg number of steps")
        plt.errorbar(train_nepisodes, test_steps_mean,
                     yerr=test_steps_std, color="navy", uplims=True, lolims=True)
        if save:
            plt.savefig(self.expt_name +
                        "test_steps_{}.png".format(len(self.test_steps)))
        else:
            plt.show()
        ...
        plt.figure(5)
        plt.clf()
        plt.title("Avg. Actor Training Loss over {} episodes".format(
            train_nepisodes[-1]))
        plt.xlabel("Number of training episodes")
        plt.ylabel("Avg. Loss")
        # plt.plot(train_losses_mean, color="crimson")
        plt.errorbar(train_nepisodes, train_losses_mean,
                     yerr=train_losses_std, color="tomato", uplims=True, lolims=True)
        if save:
            plt.savefig(
                self.expt_name + "train_loss_actor_{}.png".format(len(self.test_rewards)))
        else:
            plt.show()
        ...
        plt.figure(5)
        plt.clf()
        plt.title("Avg. Critic Training Loss over {} episodes".format(
            train_nepisodes[-1]))
        plt.xlabel("Number of training episodes")
        plt.ylabel("Avg. Loss")
        # plt.plot(train_losses_mean, color="crimson")
        ...
        if save:
            plt.savefig(
                self.expt_name + "train_loss_critic_{}.png".format(len(self.test_rewards)))
        else:
            plt.show()
        ...
        plt.figure(5)
        plt.clf()
        plt.title("Avg. AC Combined Training Loss over {} episodes".format(
            train_nepisodes[-1]))
        plt.xlabel("Number of training episodes")
        plt.ylabel("Avg. Loss")
        # plt.plot(train_losses_mean, color="crimson")
        plt.errorbar(train_nepisodes, train_losses_mean,
                     yerr=train_losses_std, color="crimson", uplims=True, lolims=True)
        if save:
            plt.savefig(self.expt_name +
                        "train_loss_{}.png".format(len(self.test_rewards)))
        else:
            plt.show()
...
def main():
    parser = argparse.ArgumentParser(
        description="Using A2C for solving LunarLander")
    parser.add_argument("--gamma", type=float, default=0.99,
                        help="discount factor (default = 0.99)")
    parser.add_argument("--lr_actor", type=float, default=5e-4,
                        help="learning rate actor (default = 1e-2)")
    parser.add_argument("--lr_critic", type=float, default=5e-4,
                        help="learning rate critic (default = 1e-2)")
    parser.add_argument("--num_episodes", type=int,
                        default=50000, help="number of episodes")
    ...
    parser.add_argument("--save_freq", type=int, default=1e4,
                        help="checkpoint frequency for saving models")
    parser.add_argument("--test_freq", type=int, default=500,
                        help="frequency for testing policy")
    parser.add_argument("--save_path", type=str,
                        default="models/a2c/", help="path for saving models")
    parser.add_argument("--expt_name", type=str,
                        default="plots/a2c/", help="expt name for saving results")
    parser.add_argument("--N_steps", type=int, default=100,
                        help="N-step for the trace")
